---
description: Worker de implementación (modelo barato). Ejecuta UNA tarea atómica de docs/roadmap.md con TDD estricto, corre los gates (just lint/test), mantiene TASKS.md y Project.canvas sincronizados (dual-write) y hace commits convencionales por checkpoint. Úsalo como agente predeterminado para construir.
mode: primary
...
3. `python3 bin/canvas-tool.py "Project.canvas" start <ID>` (o `just canvas start <ID>`) + marca `— ▶ en curso` en `TASKS.md`.
...
6. `python3 bin/canvas-tool.py "Project.canvas" finish <ID>` (o `just canvas finish <ID>`) + marca `— 🔵 en revisión` en `TASKS.md`.
